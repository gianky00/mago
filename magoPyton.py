...
def gestisci_popup_con_ocr(config, regione_screenshot, testo_da_cercare, coordinate_click, nome_popup_log):
    if not OCR_AVAILABLE:
        ...
        return False
    try:
        pytesseract.pytesseract.tesseract_cmd = config['impostazioni_file']['path_tesseract_cmd']
...
def run_automation(config):
    ...
    file_cfg = config['impostazioni_file']
    ...
    if config['impostazioni_file']['forzare_ricalcolo_excel']:
        if WIN32_AVAILABLE:
